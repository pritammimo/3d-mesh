...
function Model({ url, color, isAnimating, animSpeed, isReverse, rotationY, canvasTexture }) {
  ...
  // Calculate scale and position to fit the grid perfectly
  useEffect(() => {
    // 1. Get the bounding box of the scene
    ...
    // 2. Define a consistent target size
    const targetSize = 2.5;

    // 3. Find the maximum dimension of the object
    const maxDim = Math.max(size.x, size.y, size.z);

    // 4. Calculate scale to match target size
    const calculatedScale = maxDim > 0 ? targetSize / maxDim : 1;

    // 5. Calculate offsets to center the object at X=0, Z=0 and rest its bottom on Y=0
  ...
  //       // Base model color
  //       const baseMesh = scene.getObjectByName(child.name);
  //       if (baseMesh) {
  //         baseMesh.material = baseMesh.material.clone();
  //         baseMesh.material.color.set(color);
  //         baseMesh.material.map = null;
  //       }

  //       if (canvasTexture && child.name.toLowerCase().includes('mug')) {
  //         // Fix for the mug model UV mapping:
  //         // The exterior is mapped to a specific UV shell.
  //         // We use repeat and offset to isolate it and prevent bleeding into the interior.
  //         canvasTexture.wrapS = THREE.RepeatWrapping;
  //         canvasTexture.wrapT = THREE.ClampToEdgeWrapping;

  //         // These values are tuned for the coffee_mug.glb to cover the exterior cylinder
  //         canvasTexture.repeat.set(0.75, 0.45);
  //         canvasTexture.offset.set(0.18, 0.12);
  ...
  useEffect(() => {
    scene.traverse((child) => {
      ...
      // Your Mug.glb has 3 material groups
      // 0 = outer body
      // 1 = inner
      // 2 = handle
      if (Array.isArray(child.material)) {
        ...
        if (child.material[0]) {
          ...
          if (canvasTexture) {
            ...
            child.material[0].map = canvasTexture;
          } else {
        ...
        // =========================
        if (child.material[2]) {
          child.material[2].color.set(color);
          child.material[2].map = null;
          child.material[2].needsUpdate = true;
        }
      }
    });
  }, [scene, color, canvasTexture]);
...
export default function DesignLabMug() {
